...
def insert_document(collection, data):
    """Function to insert a form into a collection and
...
def check_type(element):
    ...
    if re.match(enums.Regex.EMAIL_PATTERN, element):
        ...
        return "phone"
    elif re.match(enums.Regex.DATE_PATTERN_DOT, element) or re.match(
        enums.Regex.DATE_PATTERN_DASH, element
    ):
        return "date"
    else:
        return "text"
        ...
        results.append(
            (
                template["name"],
                {
                    key: template[key]
                    for key in template
                    if key in elements and template[key] == elements[key]
                },
            )
        )
